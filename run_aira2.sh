#!/usr/bin/env bash
# AIRA2 — Run script for MLE-Bench competitions (no Docker)
# Usage:
#   bash run_aira2.sh prepare <competition-id>
#   bash run_aira2.sh run <competition-id> [--background] [--instances N] [--time-limit SECS]
#   bash run_aira2.sh stop <competition-id>
#   bash run_aira2.sh status <competition-id>
#   bash run_aira2.sh grade <competition-id>
...
# LLM proxy settings
export AIRA2_LLM_URL="http://127.0.0.1:8010"
export AIRA2_LLM_KEY="YOUR_API_KEY"
export AIRA2_LLM_MODEL="gemini3_pro"
export AIRA2_WORK_DIR="$WORKSPACE_DIR"
export AIRA2_PYTHON_BIN="$PYTHON"
...
ensure_proxy_running() {
    if ! curl -s "$AIRA2_LLM_URL/healthz" 2>/dev/null | grep -q "ok\|healthy\|status"; then
        ...
        sleep 5
        curl -s "$AIRA2_LLM_URL/healthz" || log "WARNING: proxy may not be ready"
    else
        log "Gemini proxy already running at $AIRA2_LLM_URL"
    fi
}
...
do_run() {
    ...
    if [[ "$background" == false ]]; then
        ...
        log "All instances complete. Run: bash run_aira2.sh grade $competition"
    fi
}
...
do_grade() {
    ...
    if [[ ! -f "$result_file" ]]; then
        ...
        log "Has the agent run yet? Try: bash run_aira2.sh status $competition"
        exit 1
    fi
...
case "$cmd" in
    ...
    *)
        ...
        echo ""
        echo "Example:"
        echo "  bash run_aira2.sh run stanford-covid-vaccine --background"
        ;;
esac
